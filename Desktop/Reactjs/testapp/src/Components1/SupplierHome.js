...
const SupplierHome = () => {
  ...
  async function getdata() {
    try {
      const item = await axios.get(`http://104.211.240.205/API/api/Supplier`)
  ...
  async function onFormSubmit(e) {
    ...
    if (data.code === '' || data.name === '' || data.address === '' || data.city === '' || data.pincode === '' || data.stateCode === '' || data.gstIn === '' || data.stateCode === '' ||
      ...
      try {
        await axios.post(`http://104.211.240.205/API/api/Supplier`, {
          ...data,
  ...
  const handleDelete = async id => {
    await axios.delete(`http://104.211.240.205/API/api/Supplier?Code=${id}`);
                ...
                items.map((value, i) =>
                  <tr key={i} >
                    ...
                    <td>
                      ...
                      <Tooltip title="Edit">
                        <IconButton><Link to={`/edit/${value.code}`}><EditIcon /></Link></IconButton>
                      </Tooltip>
                    </td>
